...
1. Edit `Sources/OriWeather/`. The manifest is `droplet.json`.
2. `droppykit build` writes `.build/OriWeather.droplet`, universal, linked against the
...
5. Put the bundle into Droppy Playground and confirm it loaded. Copy
   `.build/OriWeather.droplet` to
   `~/Library/Application Support/Droppy Playground/Droplets/ori-weather/OriWeather.droplet`,
...
and `.cursor/mcp.json` (Cursor). Codex: `codex mcp add droppykit -- ../droppykit/Scripts/droppykit mcp`.
...
- **The principal class does nothing.** `OriWeatherPrincipal` is `@objc`, is named in the
...
- **No `main.swift`.** The harness entry is `@main` in
  `Sources/OriWeatherHarness/OriWeatherHarness.swift`, and a file named `main.swift` cannot
  coexist with `@main`.
...
- **Do not edit anything under `../droppykit`.** That is the SDK checkout; fixes there go
...
- The host services a droplet calls: `../droppykit/Sources/DroppyKit/Services/DropletServices.swift`
  and `../droppykit/Sources/DroppyKit/Core/DropletHost.swift`
- The manifest type, with every field documented: `../droppykit/Sources/DroppyKit/Bundle/DropletManifest.swift`
- Design tokens and the settings components: `../droppykit/Sources/DroppyKit/DesignSystem/`
- A complete droplet that uses every surface: `../droppykit/Examples/WorldClock/`
- The compatibility promise and the version ledger: `../droppykit/COMPATIBILITY.md`
...
```
Package.swift                     product OriWeather (dynamic) and OriWeatherHarness
droplet.json                      the manifest; Info.plist is generated from it
Sources/OriWeather/              the droplet
Sources/OriWeatherHarness/       the @main harness entry; never main.swift
OriWeather.icon/                 Icon Composer document, required
Assets/Creator.png                square creator avatar, at least 256px, required
.build/OriWeather.droplet        what droppykit build writes
