...
Two things the SDK licence says that shape the board. Section 3, your droplets are yours: this repo picks its own licence and the SDK claims nothing over it. Section 4(d), the SDK may not be used to develop or assist a product that competes with Droppy's notch, island, shelf or HUD surfaces, and OriNotch is one. So the SDK checkout is opened from this repo and from nowhere else, OriNotch never reads it, and no number learnt here - `DroppyLiveActivityMetrics`, `DroppyShellMetrics`, the 70 and the 22 - is carried back across. OriNotch's D17 forbids the same thing from its own side, so the wall stands for two reasons and is checked from neither.
...
- D1, the name. Display name "Ori Weather", droplet id `ori-weather`, Swift product `OriWeather`, bundle identifier `app.getdroppy.droplet.ori-weather` (generated, never typed), preferences under `droplet.ori-weather.*`. The hyphen is what makes the scaffold spell the product `OriWeather` rather than `Oriweather`. Nothing in the name suggests Droppy publishes it, which the SDK licence's section 6 requires.
